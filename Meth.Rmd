...
Iterative proportional fitting (IPF) is a non parametric procedure to fit an array to a column and a row marginals already set with the same total count. The basic idea is to find successive row and column factors that replicates marginals (Schoen, 2003). 

The general framework is the use of log-linear models with a total count constraint, that consider the same two factor-effect (cross-product ratio). The IPF converges to MLE for every initial values  (Demig and Stephan, 1940). 
